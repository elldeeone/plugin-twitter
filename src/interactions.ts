...
export class TwitterInteractionClient {
  ...
  private async fetchThreadContext(tweet: ClientTweet): Promise<ClientTweet[]> {
    ...
    const maxDepth = this.getPositiveIntSetting(
  ...
  private getLearningTrustedUsersConfig(): string {
    return (
      (getSetting(this.runtime, "TWITTER_LEARNING_TRUSTED_USERS") as string) ||
      ""
    );
  }
  ...
  private async saveTrustedCorrection(
    ...
    try {
      ...
      logger.info(
        `Learned trusted correction from @${tweet.username} (${tweet.id})`,
      );
    } catch (error) {
      logger.warn(
        `Failed to persist trusted correction for tweet ${tweet.id}:`,
        error,
      );
    }
  }
  ...
  private async getTrustedCorrectionContext(
    ...
    const maxMemories = this.getPositiveIntSetting(
    ...
    const correctionSummaries = userFacts
      .filter((memory) => {
        const metadataType =
          typeof memory.metadata?.type === "string" ? memory.metadata.type : "";
        const contentType =
          typeof memory.content?.type === "string" ? memory.content.type : "";
        return (
          metadataType === "twitter_correction" ||
          contentType === "twitter_correction"
        );
      })
...
Response (YES or NO only):`;
  ...
  private async ensureTweetContext(tweet: ClientTweet) {
    try {
      if (!tweet?.id || !tweet.userId || !tweet.username) {
        throw new Error(
          `Tweet ${tweet?.id || "unknown"} is missing required context fields`,
        );
      }
  ...
  async processMentionTweets(mentionCandidates: ClientTweet[]) {
    ...
    for (const tweet of tweetsToProcess) {
      ...
      if (
        ...
        // Check if we've already processed this tweet before doing extra context/thread work
        ...
        await this.saveTrustedCorrection(
          tweet,
          context.entityId,
          context.roomId,
        );
  ...
  async handleTweet({
    ...
    const threadedConversation = thread?.length
      ? thread
      : await this.fetchThreadContext(tweet);
